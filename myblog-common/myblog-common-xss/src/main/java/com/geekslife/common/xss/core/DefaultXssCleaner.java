...
import com.geekslife.common.xss.config.MyBlogXssProperties;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final MyBlogXssProperties properties;

	public DefaultXssCleaner(MyBlogXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(MyBlogXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		MyBlogXssProperties.Mode mode = properties.getMode();
		if (MyBlogXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (MyBlogXssProperties.Mode.validate == mode) {
			// 校验
