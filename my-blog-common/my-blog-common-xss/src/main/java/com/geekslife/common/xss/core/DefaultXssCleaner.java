...
package com.geekslife.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.geekslife.common.xss.config.My-BlogXssProperties;
import com.geekslife.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final My-BlogXssProperties properties;

	public DefaultXssCleaner(My-BlogXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(My-BlogXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		My-BlogXssProperties.Mode mode = properties.getMode();
		if (My-BlogXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (My-BlogXssProperties.Mode.validate == mode) {
			// 校验
